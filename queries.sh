...
echo "$($PSQL "SELECT name FROM teams WHERE team_id = 1")"
...
echo "$($PSQL "SELECT DISTINCT g.year, t.name FROM teams t JOIN games g ON (t.team_id = g.winner_id) WHERE round = 'Final' ORDER BY g.year")"

echo -e "\nList of teams that start with 'Co':"
echo "$($PSQL "SELECT DISTINCT t.name FROM teams t JOIN games g ON (t.team_id = g.winner_id) WHERE name LIKE 'Co%' ORDER BY t.name")"
